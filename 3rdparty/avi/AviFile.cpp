/*  CAviFile by P.Gopalakrishna (we're outsourcing!)
...
HRESULT	CAviFile::AppendSound(LPVOID pAudBuf,int numbytes) {
  ...
  if(numbytes>wfx.nAvgBytesPerSec/shifter_freq_at_start_of_vbl)
    numbytes=wfx.nAvgBytesPerSec/shifter_freq_at_start_of_vbl;
...
#endif //defined(STEVEN_SEAGAL) && defined(SS_VID_RECORD_AVI)