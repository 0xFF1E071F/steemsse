/*
  Based on C source by Maverick (Fabio Bizzetti) found at
  http://membres.multimania.fr/amycoders/tutorials/maverick/sfftm.html
  changed in handier C++ classes because it was screeming for object
  handling instead of strange code duplication (don't understand what
  he meant exactly!)
  Used in Steem SSE for STE Microwire emulation.
  It works more or less but I had bad crashes (float library error) in
  the treble section, then later when starting with no INI file
