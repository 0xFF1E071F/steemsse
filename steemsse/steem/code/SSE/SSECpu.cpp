...
/*This signal is generated by an internal ring counter that may
come up in any state. (At power-on, it is impossible to guarantee
phase relationship of E to CLK.)
*/
    eclock_sync_cycle=rand()%10;
#endif
  }
#endif
...
#if defined(SSE_CPU_ECLOCK_SIMPLIFY)
/*  No reason to have blocks of 4 cycles (0, 4 or 8), the real value is 0-9.
    Every 10 CPU cycles, there's sync (0 wait state), which can be any of the 
    cycles. Before and after that there's 9...1 wait states.
    This is conform to M68000 UM.
    We renounce the "STF WS1" hacks, they don't seem to make sense.
    Problem: the rest of emulation isn't up to this ambition.
*/

int TM68000::SyncEClock() {
  ASSERT(eclock_sync_cycle<10);
  UpdateCyclesForEClock();
#if defined(SSE_CPU_ECLOCK_SIMPLIFYB)
/*  Compromise: even cycles only because other aspects of emulation are 
    aligned.
*/
  BYTE cycles=( (cycles_for_eclock+eclock_sync_cycle)&~1 )%10;
  BYTE wait_states=8-cycles; 
#else
  BYTE cycles=(cycles_for_eclock+eclock_sync_cycle)%10;
  BYTE wait_states=9-cycles; 
#endif
  return wait_states;
}
...
int TM68000::SyncEClock(int dispatcher) {
  ...
  case 0:
    wait_states=8;
#if defined(SSE_INT_HBL_E_CLOCK_HACK_382) //TODO
