...
void ASMCALL Blitter_Start_Now()
{
...
#if defined(SSE_BLT_CPU_RUNNING) && !defined(SSE_BLT_MAIN_LOOP)
  Blit.TimeAtBlit=ACT;
#endif
#if defined(SSE_BLT_BUS_ARBITRATION)
  Blit.Request=0;
  ...
  if (WORD(Blit.XCounter-Blit.XCount)==0) 
    Blitter_Start_Line();
  Blitter_Draw();
...
#if defined(SSE_BLT_BUS_ARBITRATION) // TODO
  int blit_bus_cycles=(
...
old_pc,TIMING_INFO,Val,Blit.Hop,Blit.Op,Blit.XCount,Blit.YCount,Blit.SrcAdr,Blit.SrcXInc,Blit.SrcYInc,Blit.DestAdr,Blit.DestXInc,Blit.DestYInc,Blit.NFSR,Blit.FXSR,Blit.Skew,Blit.EndMask[0],Blit.EndMask[1],Blit.EndMask[2]);
          ...
          if(!Blit.HasBus) // avoid stack overflow...
            Blitter_Draw();
#else//!main loop
          Blit.HasBus=true;
