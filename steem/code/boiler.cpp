...
#if !defined(SSE_BOILER_NO_SET_BRK_PC_MENU)
      ...
      if (HIWORD(wPar)==STN_CLICKED || HIWORD(wPar)==BN_CLICKED){
        ...
        //            fread(Mem+0x400,prg_len,1,sf);
            ...
            case 1523: 
              ...
              break;
#endif
#if defined(SSE_BOILER_DUMP_6301_RAM) && defined(SSE_ACIA)
            case 1524:
              hd6301_dump_ram();
              break;
#endif
...
#if defined(SSE_BOILER_BROWSER_DMASOUND)
            case 913:
...
#if defined(SSE_BOILER_BROWSER_ACIA)
            case 916:
...
void disa_to_file(FILE*f,MEM_ADDRESS dstart,int dlen,bool as_source)
{
  ...
  while (dpc<dend){
    ...
    if(!f)
    {
      ...
      if(buffer_ptr-buffer>dlen*multiplier)
      {
        delete [] buffer; // horrible C++ syntax
        return;
      }
    }
    else
...
#else
#if defined(SSE_BOILER_MOD_REGS)
...
#if defined(SSE_BOILER_SHOW_ECLOCK) && !defined(SSE_CPU_ECLOCK_SIMPLIFY)
    y-=30;
    x+=70;
